...
import { memberProfilePolicy, resolveDisplayName } from "@/features/member-profile/member-profile-policy";
...
export const memberProfileService = {
  ...
  async updateSkills(
    ...
    await inSerializableTransaction(async (tx) => {
      ...
      const toRemove = existing.filter((row) => row.deletedAt === null && !desiredSet.has(row.skillId));
